...
from transformers import RTDetrV2Config, RTDetrV2ForObjectDetection, DetrImageProcessor
from torch.optim import AdamW
# 🌟 核心修改 1: 改用 CosineAnnealingLR (餘弦退火) 讓學習率平滑下降
...
def collate_fn(batch):
    ...
    batch_dict = processor(images=images, annotations=targets, return_tensors="pt")
    return batch_dict
...
def main():
    ...
    train_transform = A.Compose([
        ...
        A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.1, rotate_limit=10, p=0.5, border_mode=cv2.BORDER_CONSTANT, value=0),
        A.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1, p=0.8),
        A.GaussianBlur(blur_limit=3, p=0.2),
    ], bbox_params=A.BboxParams(format='coco', label_fields=['class_labels'], min_area=0.1)) # 移除 min_visibility，保留極小 min_area=0.1 防呆
    ...
    ], bbox_params=A.BboxParams(format='coco', label_fields=['class_labels'], min_area=0.1))
    ...
    train_dataset = DigitDataset(img_dir=train_dir, annotation_file=train_json, transform=train_transform)
    val_dataset = DigitDataset(img_dir=val_dir, annotation_file=val_json, transform=val_transform)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=collate_fn, num_workers=4, pin_memory=True, persistent_workers=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            pixel_mask = batch["pixel_mask"].to(device, non_blocking=True) if "pixel_mask" in batch else None
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0 or (batch_idx + 1) == train_total_batches:
            ...
            if (batch_idx + 1) % 100 == 0 or (batch_idx + 1) == train_total_batches:
                current_lr = optimizer.param_groups[0]['lr']
                current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                print(f"[{current_time}] Train | Epoch: [{epoch+1}/{num_epochs}] | Batch: [{batch_idx+1}/{train_total_batches}] | Loss: {outputs.loss.item():.4f} | LR: {current_lr:.6f}")
        ...
        print(f"--> Epoch {epoch+1} 總結 | Train Loss: {train_avg_loss:.4f} | Val Loss: {val_avg_loss:.4f}")
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f"=> [固定儲存] 達到 5 的倍數 Epoch，模型已額外儲存為 ({fixed_save_path})")
