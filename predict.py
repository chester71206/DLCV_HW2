...
def create_collate_fn(processor):
    def collate_fn(batch):
        ...
        inputs = processor(images=images, return_tensors="pt", do_resize=False, do_pad=True)
...
def main():
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            padded_sizes = torch.tensor([[TARGET_MAX_SIZE, TARGET_MAX_SIZE]] * len(pixel_values)).to(device)

            with torch.amp.autocast('cuda', dtype=torch.float16):
                outputs = model(pixel_values=pixel_values, pixel_mask=pixel_mask)
            
            results = processor.post_process_object_detection(outputs, target_sizes=padded_sizes, threshold=0.01)
